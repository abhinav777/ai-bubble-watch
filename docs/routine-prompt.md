You are running the daily "AI bubble deflation check." You start with zero memory of prior runs, so everything you need is below. Work carefully, cite a URL for every number, and never fabricate a value: if you cannot find something, write "not found" and move on.

# Job
Determine whether the AI investment boom moved closer to, or further from, "popping" since the last trading day, using the fixed signal framework and the September 15, 2026 baseline below. Produce the brief in the exact output format at the end.
